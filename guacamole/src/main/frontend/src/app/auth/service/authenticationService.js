...
angular.module('auth').factory('authenticationService', ['$injector',
     ...
     *     successful.
     */
    service.logout = async function logout() {
        try {
            const response = await fetch("/guacamole/api/tokens/env");
            const data = await response.json();
            const openIdLogoutUrl = data.OPENID_LOGOUT_REDIRECT;
            const openIdLoginUrl = data.OPENID_REDIRECT_URI;
            
            // Check if openIdLogoutUrl is defined and not null
            if (openIdLogoutUrl) {
                window.location.href = openIdLogoutUrl;
                window.location.href = openIdLoginUrl
            } else {
                console.error("OpenID Logout URL is not defined or null.");
                return; // Exit the function if URL is not defined or null
            }
